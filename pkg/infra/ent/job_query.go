...
func (jq *JobQuery) QueryAlert() *AlertQuery {
	query := &AlertQuery{config: jq.config}
...
func (jq *JobQuery) QueryActionLogs() *ActionLogQuery {
	query := &ActionLogQuery{config: jq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := jq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (jq *JobQuery) Clone() *JobQuery {
	...
	return &JobQuery{
		...
		withAlert:      jq.withAlert.Clone(),
		withActionLogs: jq.withActionLogs.Clone(),
...
func (jq *JobQuery) WithAlert(opts ...func(*AlertQuery)) *JobQuery {
	query := &AlertQuery{config: jq.config}
...
func (jq *JobQuery) WithActionLogs(opts ...func(*ActionLogQuery)) *JobQuery {
	query := &ActionLogQuery{config: jq.config}
	for _, opt := range opts {
		opt(query)
	}
	jq.withActionLogs = query
	return jq
}
...
func (jq *JobQuery) sqlAll(ctx context.Context) ([]*Job, error) {
	...
	if query := jq.withAlert; query != nil {
		ids := make([]types.AlertID, 0, len(nodes))
		nodeids := make(map[types.AlertID][]*Job)
		for i := range nodes {
			if nodes[i].alert_jobs == nil {
				continue
			}
			fk := *nodes[i].alert_jobs
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "alert_jobs" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := jq.withActionLogs; query != nil {
		...
		query.Where(predicate.ActionLog(func(s *sql.Selector) {
			s.Where(sql.InValues(job.ActionLogsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.job_action_logs
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "job_action_logs" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "job_action_logs" returned %v for node %v`, *fk, n.ID)
			}
